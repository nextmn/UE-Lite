...
func main() {
	...
	app := &cli.Command{
		Name:                  "ue-lite",
		Usage:                 "NextMN-UE Lite - Experimental UE Simulator",
		...
		Flags: []cli.Flag{
			...
			&cli.StringFlag{
				...
				Sources: cli.NewValueSourceChain(
					...
					clixdg.ConfigFile("nextmn-ue-lite/config.yaml"),
				),
			},
		},
		DefaultCommand: "run",
		Commands: []*cli.Command{
			...
			},
			{
				...
				Action: func(ctx context.Context, cmd *cli.Command) error {
					...
					if err := healthcheck.NewHealthcheck(*conf.Control.Uri.JoinPath("status"), "go-github-nextmn-ue-lite").Run(ctx); err != nil {
						os.Exit(1)
					}
					return nil
				},
